...
def SmartAnalyzeTransient(dt, npts, ud=""):
    ...
    test(
        control["testType"],
        control["testTol"],
        control["testIterTimes"],
        control["testPrintFlag"],
    )
    ...
    for seg in range(1, npts + 1):
        ...
        if ok < 0:
            print(
                ">>> SmartAnalyze: Analyze failed. Time consumption: %f s."
        ...
        if control["debugMode"]:
            print(
                "*** SmartAnalyze: progress %f"
                % (current["progress"] / current["segs"])
            )

    # 全部数据点分析成功，显示分析成功和用时
    print(
        ">>> SmartAnalyze: Successfully finished! Time consumption: %f s."
        % (time.time() - current["startTime"])
    )
...
def SmartAnalyzeStatic(node, dof, maxStep, targets, ud=""):
    ...
    for i in range(len(targets)):  # 目标位移列表循环
        ...
        if positive:  # 若目前处于正向加载
            ...
            while (
                section - j * maxStep
            ) > maxStep:  # 把本段推覆位移分为长度为maxStep的各个小段，存于列表segs中
                segs.append(maxStep)
                j += 1
    ...
    for seg in segs:
        ok = RecursiveAnalyze(
            seg, 0, control["testIterTimes"], control["testTol"], control, current
        )
...
def RecursiveAnalyze(step, algoIndex, testIterTimes, testTol, vcontrol, vcurrent):
    ...
    if control["debugMode"]:
        print(
            "*** SmartAnalyze: Run Recursive: step=%f, algoI=%i, times=%i, tol=%f"
    ...
    if algoIndex != current["algoIndex"]:
        print(
            ">>> SmartAnalyze: Setting algorithm to %i"
            % (control["algoTypes"][algoIndex])
        )
        print("\n")
    ...
    if testIterTimes != current["testIterTimes"] or testTol != current["testTol"]:
        if testIterTimes != current["testIterTimes"]:
            print(
                ">>> SmartAnalyze: Setting test iteration times to %i" % (testIterTimes)
            )
            print("\n")
    ...
    if ok == 0:
        ...
        if current["counter"] >= control["printPer"]:
            print(
                "* SmartAnalyze: progress %f. Time consumption: %f s."
                % (
                    current["progress"] / current["segs"],
                    (time.time() - current["startTime"]) / 1000.0,
                )
            )
            print("\n")
            current["counter"] = 0
    ...
    if control["tryAddTestTimes"] and testIterTimes != control["testIterTimesMore"]:
        ...
        if norm[-1] < control["normTol"]:
            print(
                ">>> SmartAnalyze: Adding test times to %i."
                % (control["testIterTimesMore"])
            )
            print("\n")
            return RecursiveAnalyze(
                step, algoIndex, control["testIterTimesMore"], testTol, control, current
            )
        # 如果范数大于规定值，不增加
        else:
    ...
    if control["tryAlterAlgoTypes"] and (algoIndex + 1) < len(control["algoTypes"]):
        ...
        print("\n")
        return RecursiveAnalyze(
            step, algoIndex, testIterTimes, testTol, control, current
        )
    ...
    if abs(step) < 2 * control["minStep"]:
        ...
        print("\n")
        if (
            control["tryLooseTestTol"]
            and current["testTol"] != control["looseTestTolTo"]
        ):
    ...
    print(
        ">>> SmartAnalyze: Dividing the current step %f into %f and %f"
...
def setAlgorithm(algotype):
    ...
    def case21():
        print(
            "> SmartAnalyze: Setting algorithm to  NewtonLineSearch -type Bisection ..."
        )
    ...
    def case23():
        print(
            "> SmartAnalyze: Setting algorithm to  NewtonLineSearch -type RegulaFalsi ..."
        )
    ...
    def case42():
        print(
            "> SmartAnalyze: Setting algorithm to  KrylovNewton -increment initial ..."
        )
        algorithm("KrylovNewton", increment="initial")

    def case43():
        print(
            "> SmartAnalyze: Setting algorithm to  KrylovNewton -iterate initial -increment initial ..."
        )
    ...
    def case45():
        print(
            "> SmartAnalyze: Setting algorithm to  KrylovNewton -iterate initial -increment initial -maxDim 50"
        )
    ...
    def case52():
        print(
            "> SmartAnalyze: Setting algorithm to  SecantNewton -increment initial  ..."
        )
        algorithm("SecantNewton", increment="initial")

    def case53():
        print(
            "> SmartAnalyze: Setting algorithm to  SecantNewton -iterate initial -increment initial ..."
        )
...
def build_protocol(cycle_number, cycle_incr, incr) -> list:
    ans = [0]
    for _i in range(1, cycle_number + 1):
        u = int(cycle_incr / incr) * _i
        for j in range(0, u):
            ans.append(ans[-1] + incr)
        for j in range(0, 2 * u):
            ans.append(ans[-1] - incr)
        for j in range(0, u):
            ans.append(ans[-1] + incr)
